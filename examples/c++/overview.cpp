...
int main() {
	...
	if(mode == TRAIN)
	{
		...
		//Array<float, 2, 2> X = allocate(3 * SIZE_DATASET*(N_SAMPLES_0 + N_SAMPLES_1 + N_SAMPLES_2), n_features);
		//Array<uint, 2, 2> Y = allocate(3 * SIZE_DATASET*(N_SAMPLES_0 + N_SAMPLES_1 + N_SAMPLES_2), 1);
...
#pragma omp parallel for num_threads(N_THREADS) \
			...
			// UPSAMPLE USING JOINT BILATERAL FILTER: advantages of joint bilateral upsampling are already exploited 
			...
			cv::inRange(sensor_depth, depth_hand - depth_range, /*mm*/
			...
			cv::Mat element_erosion = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(2 * (DILATION_SIZE+1) + 1, 2 * (DILATION_SIZE + 1) + 1));

			cv::dilate(mask, mask, element);
			
			
			//cv::dilate(mask_wrist, mask_wrist, element);
			//cv::erode(mask_wrist, mask_wrist, element_erosion);
			...
			// TO DO: add connected component pixels taking inspiration from old algo
			if (depth_hand > 10000*DEPTH_PP)
			{
				// Extra post processing needed when hand is far away!
				std::cout << "extra post processing" << std::endl;

				Eigen::Vector3f _wband_center = Eigen::Vector3f(0, 0, 0);
				...
				// TIMED_BLOCK(timer,"Worker_classify::(PCA)")
				{
					...
					for (int row = 0; row < mask.rows; ++row) {
						for (int col = 0; col < mask.cols; ++col) {
							if (mask.at<uchar>(row, col) != 120) continue;
							_wband_center += point_at_depth_pixel(sensor_depth, col, row, iproj);
							counter++;
						}
					}
					...
					for (int row = 0; row < mask.rows; ++row) {
						for (int col = 0; col < mask.cols; ++col) {
							if (mask.at<uchar>(row, col) != 120) continue;
							Eigen::Vector3f p_pixel = point_at_depth_pixel(sensor_depth, col, row, iproj);
					...
					///--- Compute PCA
					Eigen::Map<Eigen::Matrix<float, 3, -1>> points_mat(points_pca[0].data(), 3, points_pca.size());
					for (int i : {0, 1, 2})
						points_mat.row(i).array() -= _wband_center(i);
					Eigen::Matrix<float, 3, 3> cov = points_mat*points_mat.adjoint();
					Eigen::SelfAdjointEigenSolver<Eigen::Matrix<float, 3, 3>> eig(cov);
				...
				std::cout << _wband_center << std::endl;

				// TIMED_BLOCK(timer,"Worker_classify::(in sphere)")
				{
					float crop_radius_sq = crop_radius * crop_radius;
					Eigen::Vector3f crop_center = _wband_center + _wband_dir*crop_radius;
					//Eigen::Vector3f crop_center = _wband_center + Eigen::Vector3f(0,1,0)*crop_radius;
					//Vector3 crop_center = _wband_center + _wband_dir*( crop_radius + wband_size /*mm*/);

					for (int row = 0; row < mask.rows; ++row) {
						for (int col = 0; col < mask.cols; ++col) {
							if (range_mask.at<uchar>(row, col) != 255 || mask.at<uchar>(row, col) != 0) continue;
			...
			//probabilityMap_us.convertTo(mask, CV_8UC1, 255);
			//cv::imshow("predicted", probabilityMap>THRESHOLD);
			cv::imshow("without additional pp", mask);
			//cv::imshow("predicted_us", dst);
		}
	}
